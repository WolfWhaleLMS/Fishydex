...
### iNaturalist API
- When you use the species identification feature, the photo you capture is sent to the iNaturalist API (operated by the California Academy of Sciences and National Geographic Society) for automated species identification.
- iNaturalist processes the image to return species suggestions and confidence scores.
- We do not control how iNaturalist handles images sent to their API. Please refer to the [iNaturalist Privacy Policy](https://www.inaturalist.org/pages/privacy) for details on their data practices.
- No personal information (name, account details, location) is sent to iNaturalist -- only the photo image data.

## Data Storage

- All catch data (species, date, location, photos, notes) is stored locally on your device using SwiftData.
- There are no user accounts. You do not need to sign up or log in.
- No data is stored on our servers. We do not operate a backend database for user data.
