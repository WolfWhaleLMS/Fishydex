...
/// Applies a Fortnite-style glow effect to any view based on rarity tier.
struct RarityGlowModifier: ViewModifier {
    ...
    func body(content: Content) -> some View {
        ...
        case .legendary:
            content
                .shadow(
                    color: tier.swiftUIColor.opacity(animating ? 0.7 : 0.15),
                    radius: animating ? 16 : 6
                )
                .shadow(
                    color: tier.swiftUIColor.opacity(animating ? 0.4 : 0.05),
                    radius: animating ? 28 : 10
                )
                .animation(
        ...
        case .mythic:
            content
                .overlay(
                    MythicGlowEffect(animating: $animating)
                        .allowsHitTesting(false)
                )
                .shadow(
                    color: tier.swiftUIColor.opacity(animating ? 0.8 : 0.2),
                    radius: animating ? 20 : 8
                )
                .shadow(
                    color: tier.swiftUIColor.opacity(animating ? 0.5 : 0.1),
                    radius: animating ? 36 : 14
                )
                .animation(
...
// MARK: - Mythic Particle Glow

/// A dedicated overlay for mythic-tier items that simulates gold particles radiating outward.
private struct MythicGlowEffect: View {
                ...
                ForEach(0..<particleCount, id: \.self) { index in
                    ...
                    Circle()
                        .fill(
                            RadialGradient(
                                colors: [
                                    Color.rarityMythic.opacity(animating ? 0.9 : 0.2),
                                    Color.rarityMythic.opacity(0)
                                ],
                                center: .center,
                                startRadius: 0,
                                endRadius: 8
